...
export default function InviteJoinPage() {
  ...
  async function handleLogin() {
    ...
    await supabase.auth.signInWithOAuth({
      provider: 'google',
      options: {
        redirectTo: `${window.location.origin}/auth/callback?next=/invite/accept?token=${token}`,
      },
    })
  }
  ...
  if (!token) {
    return (
      <main className="min-h-screen flex items-center justify-center bg-background">
        <p className="text-muted-foreground">הזמנה לא תקינה</p>
      </main>
    )
  }
      ...
      <div className="flex flex-col items-center gap-8 p-8 rounded-3xl bg-card border border-border w-full max-w-sm shadow-2xl">
        <div className="flex flex-col items-center gap-2 text-center">
          ...
          <p className="text-muted-foreground text-sm">
            הוזמנת להצטרף לתיק ההשקעות המשפחתי!
          </p>
        </div>
